...
if(sizeof($arIds) > 0)
{
    ...
    while($arItem = $rsFile->GetNext())
    {
        ...
        $urlPreviewPicture = itc\Resizer::get($arItem['ID'], 'newsPicture');
    ...
    foreach($arResult['ITEMS'] as &$arItem)
    {
        ...
        {
            $arItem['PREVIEW_PICTURE']['SRC'] = itc\Resizer::get($environment->get('newsPlugId'), 'newsPicture');
            $arItem['DETAIL_PICTURE']['SRC']  = itc\Resizer::get($environment->get('newsPlugId'), 'newsPicture');
        }
    }
    unset($arItem);
}
