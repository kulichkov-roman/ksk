<?php
/**
 * Создает инфоблок «Каталог: полуфабрикаты»
 *
 ...
 */
class CreateCatalogSmfIBlockMigration extends AbstractIBlockMigration
{
    /**
     * {@inheritdoc}
     */
    public function up()
    {
        ...
        try {
            ...
            $logger->log(
                sprintf('IBlock has been created. Id: "%s". Add to "catalogSmfIBlockId"', $this->iblockId)
            );
    ...
    public function down()
    {
        ...
        $this->deleteIBlock($environment->get('catalogSmfIBlockId'));

        $logger->log(sprintf('IBlock rewards has been removed. Id: "%s"', $this->iblockId));
    }
}

$migration = new CreateCatalogSmfIBlockMigration();
$migration->up();
?>
