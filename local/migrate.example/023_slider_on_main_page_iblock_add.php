<?
/**
 * Создает инфоблок для слайдера на главной странице
 */
...
/**
 * Class CreateSlideMainIblock
 */
class CreateSlideMainIblock implements MigrationInterface
{
	/**
	...
	{
		$ib = new CIBlock;
		$dbIbRes = $ib->GetList(array(), array('CODE' => 'main_slider', 'IBLOCK_TYPE_ID' => 'services'));
		if (!$arIbRes = $dbIbRes->Fetch()) {
			...
			$ID = $ib->Add($arFields);
			if ($ID) {
				$this->logger->log(sprintf('Iblock "%s" (%s) has been added', 'main_slider', $ID));
			} else {
				$this->logger->log(sprintf('Unable to add Iblock "%s" (%s) Error:%s', 'main_slider', $ID,
					$ib->LAST_ERROR));
			}
		} else {
			$this->logger->log(sprintf('Iblock "%s" (%s) already exist', $arIbRes['CODE'], $arIbRes['ID']));
		}
	}
...
$migrationIblockCreate = new CreateSlideMainIblock();
