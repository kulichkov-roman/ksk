 ...
 */
class CreateActionIblock implements MigrationInterface
{
	/**
	...
	{
		$ib = new CIBlock;
		$dbIbRes = $ib->GetList(array(), array('CODE' => 'action', 'IBLOCK_TYPE_ID' => 'services'));
		if (!$arIbRes = $dbIbRes->Fetch()) {
			...
			$ID = $ib->Add($arFields);
			if ($ID) {
				$this->logger->log(sprintf('Iblock "%s" (%s) has been added', 'action', $ID));
			} else {
				$this->logger->log(sprintf('Unable to add Iblock "%s" (%s) Error:%s', 'action', $ID,
					$ib->LAST_ERROR));
			}
		} else {
...
$migrationIblockCreate = new CreateActionIblock();
